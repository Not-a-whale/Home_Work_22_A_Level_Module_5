...
export const DataManagementService = {
    ...
    getAllProducts: async (cat?: string) => {
        axios.get(cat ? `http://localhost:3000/products?category=${cat}` : "http://localhost:3000/products")
        .then((items) => {
    ...
    buy: async (cartItems: {id: string, quantity: number}[]) => {
        ...
        cartItems.forEach(cartItem => {
            ...
            axios.patch("http://localhost:3000/products/" + cartItem.id, pathObject).catch((err) => {
                alert(err.message);
            })
        });
